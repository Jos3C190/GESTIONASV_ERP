...
@router.post(
    ...
    dependencies=[Depends(require_permission("products:master_data"))],
)
async def create_brand(
    ...
    if await session.scalar(
        select(ProductBrandModel).where(
            ProductBrandModel.company_id == company_id,
            ProductBrandModel.normalized_name == normalized,
        )
    ):
    ...
    await audit.record(
        ...
        user_id=current.id,
        company_id=company_id,
        resource_type="product_brands",
        resource_id=str(row.id),
        after_state={"code": row.code, "name": row.name},
    )
...
async def update_brand(
    ...
    row = await session.scalar(
        select(ProductBrandModel)
        .where(ProductBrandModel.id == brand_id, ProductBrandModel.company_id == company_id)
        .with_for_update()
    )
    if row is None:
...
@router.get(
    ...
    response_model=list[ProductManufacturerResponse],
    dependencies=[Depends(require_permission("products:read"))],
)
async def list_manufacturers(
    request: Request, session: SessionDep, current: CurrentUser
) -> list[ProductManufacturerResponse]:
    company_id = _company(request)
    await require_company_access(session, current, company_id)
    rows = (
        await session.scalars(
            select(ProductManufacturerModel)
            .where(ProductManufacturerModel.company_id == company_id)
            .order_by(ProductManufacturerModel.legal_name)
        )
    ).all()
...
@router.post(
    "/catalog/manufacturers",
    response_model=ProductManufacturerResponse,
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(require_permission("products:master_data"))],
)
async def create_manufacturer(
    payload: ProductManufacturerCreate,
    request: Request,
    session: SessionDep,
    current: CurrentUser,
    audit: AuditService = Depends(get_audit_service),
    ...
    await audit.record(
        ...
        user_id=current.id,
        company_id=company_id,
        resource_type="product_manufacturers",
        resource_id=str(row.id),
        after_state={"legal_name": row.legal_name},
    )
    return ProductManufacturerResponse.model_validate(row, from_attributes=True)


@router.patch(
    "/catalog/manufacturers/{manufacturer_id}",
    response_model=ProductManufacturerResponse,
    dependencies=[Depends(require_permission("products:master_data"))],
)
async def update_manufacturer(
    manufacturer_id: uuid.UUID,
    payload: ProductManufacturerUpdate,
    request: Request,
    session: SessionDep,
    current: CurrentUser,
    audit: AuditService = Depends(get_audit_service),
    ...
    row = await session.scalar(
        select(ProductManufacturerModel)
        .where(
            ProductManufacturerModel.id == manufacturer_id,
            ProductManufacturerModel.company_id == company_id,
        )
        .with_for_update()
    )
    if row is None:
...
async def _product(
    session: SessionDep, company_id: uuid.UUID, product_id: int, *, lock: bool = False
) -> ProductModel:
...
@router.get(
    "/catalog/products/{product_id}/identifiers",
    response_model=list[ProductIdentifierResponse],
    dependencies=[Depends(require_permission("products:read"))],
)
async def list_identifiers(
    product_id: int, request: Request, session: SessionDep, current: CurrentUser
) -> list[ProductIdentifierResponse]:
    company_id = _company(request)
    await require_company_access(session, current, company_id)
    ...
    rows = (
        await session.scalars(
            ...
            .order_by(ProductIdentifierModel.identifier_type, ProductIdentifierModel.value)
        )
    ).all()
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(require_permission("products:identifiers"))],
)
async def create_identifier(
    ...
    if await session.scalar(
        select(ProductIdentifierModel).where(
            ProductIdentifierModel.company_id == company_id,
            ProductIdentifierModel.identifier_type == payload.identifier_type,
            ProductIdentifierModel.normalized_value == normalized,
        )
    ):
        raise HTTPException(
            status_code=409, detail="El identificador ya está registrado en esta empresa."
        )
    ...
    await audit.record(
        ...
        user_id=current.id,
        company_id=company_id,
        resource_type="product_identifiers",
...
@router.patch(
    "/catalog/products/{product_id}/identifiers/{identifier_id}",
    response_model=ProductIdentifierResponse,
    dependencies=[Depends(require_permission("products:identifiers"))],
)
...
async def _validate_supplier(
    session: SessionDep, company_id: uuid.UUID, supplier_id: int
) -> SupplierModel:
...
async def replace_product_suppliers(  # noqa: C901
    ...
    if len(incoming_ids) != len(set(incoming_ids)):
        raise HTTPException(
            status_code=409, detail="Un proveedor no puede repetirse en el mismo producto."
        )
    ...
    for item in payload.suppliers:
        ...
        if (
            ...
            and (supplier.supplier_status != "approved" or not supplier.is_active)
        ):
            raise HTTPException(
                status_code=409, detail="Solo un proveedor activo y aprobado puede ser preferido."
            )

    desired_ids = set(incoming_ids)
    for stale_item in existing:
        if stale_item.supplier_id not in desired_ids:
    ...
    for incoming in payload.suppliers:
        ...
        existing_item = existing_by_supplier.get(incoming.supplier_id)
...
async def create_product_supplier(
    ...
    existing = await session.scalar(
        select(ProductSupplierModel).where(
            ProductSupplierModel.company_id == company_id,
            ProductSupplierModel.product_id == product_id,
            ProductSupplierModel.supplier_id == payload.supplier_id,
        )
    )
    if existing:
        raise HTTPException(
            status_code=409, detail="El proveedor ya está relacionado con este producto."
        )
    if payload.is_preferred:
        await session.execute(
            update(ProductSupplierModel)
            .where(ProductSupplierModel.product_id == product_id)
            .values(is_preferred=False)
        )
    item = ProductSupplierModel(
        company_id=company_id, product_id=product_id, **payload.model_dump()
    )
    session.add(item)
    await session.flush()
    await audit.record(
...
@router.patch(
    "/catalog/products/{product_id}/suppliers/{relation_id}",
    response_model=ProductSupplierResponse,
    dependencies=[Depends(require_permission("products:suppliers"))],
)
async def update_product_supplier(
    ...
    item = await session.scalar(
        select(ProductSupplierModel)
        .where(
            ProductSupplierModel.id == relation_id,
            ProductSupplierModel.company_id == company_id,
            ProductSupplierModel.product_id == product_id,
        )
        .with_for_update()
    ...
    data = payload.model_dump(exclude_unset=True)
    supplier = (
        await _validate_supplier(session, company_id, int(data["supplier_id"]))
        if "supplier_id" in data and data["supplier_id"] is not None
        else await _validate_supplier(session, company_id, item.supplier_id)
    )
...
async def delete_product_supplier(
    ...
    await audit.record(
        ...
        after_state={"product_id": product_id, "supplier_id": item.supplier_id},
    )
...
async def list_supplier_products(
    ...
    rows = (
        await session.scalars(
            ...
            .order_by(ProductSupplierModel.created_at)
            .limit(limit)
        )
    ).all()
    return [_supplier(item) for item in rows]
