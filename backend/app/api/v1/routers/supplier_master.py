...
async def _ensure_supplier(
    repo: SupplierMasterRepository, company_id: uuid.UUID, supplier_id: int
) -> None:
...
async def list_currencies(
    session: SessionDep, current: CurrentUser, request: Request
) -> list[CurrencyResponse]:
    await require_company_access(session, current, request_company_id(request))
    return [
        CurrencyResponse.model_validate(item) for item in await _repo(session).list_currencies()
    ]


@router.get(
    "/supplier-groups",
    response_model=list[SupplierGroupResponse],
    dependencies=[Depends(require_permission("suppliers:read"))],
)
async def list_supplier_groups(
    request: Request, session: SessionDep, current: CurrentUser
) -> list[SupplierGroupResponse]:
    company_id = request_company_id(request)
    await require_company_access(session, current, company_id)
    return [
        SupplierGroupResponse.model_validate(item)
        for item in await _repo(session).list_groups(company_id)
    ]


@router.post(
    "/supplier-groups",
    response_model=SupplierGroupResponse,
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(require_permission("suppliers:manage"))],
)
async def create_supplier_group(
    payload: SupplierGroupCreate,
    request: Request,
    session: SessionDep,
    current: CurrentUser,
    ...
    await audit.record(
        ...
        resource_type="supplier_groups",
        resource_id=str(item.id),
        after_state={"code": item.code, "name": item.name},
    )
    return SupplierGroupResponse.model_validate(item)


@router.patch(
    "/supplier-groups/{group_id}",
    response_model=SupplierGroupResponse,
    dependencies=[Depends(require_permission("suppliers:manage"))],
)
async def update_supplier_group(
    group_id: uuid.UUID,
    payload: SupplierGroupUpdate,
    request: Request,
    session: SessionDep,
    current: CurrentUser,
    audit: AuditService = Depends(get_audit_service),
    ...
    await audit.record(
        ...
        resource_id=str(item.id),
        before_state=before,
        after_state={"code": item.code, "name": item.name, "is_active": item.is_active},
    )
    return SupplierGroupResponse.model_validate(item)
...
async def list_payment_terms(
    ...
    await require_company_access(session, current, company_id)
    return [
        PaymentTermsResponse.model_validate(item)
        for item in await _repo(session).list_payment_terms(company_id)
    ]
...
async def create_payment_terms(
    ...
    if payload.discount_days > payload.net_days:
        raise HTTPException(
            status_code=422, detail="Los días de descuento no pueden superar los días netos"
        )
    ...
    await audit.record(
        ...
        resource_id=str(item.id),
        after_state={"code": item.code, "net_days": item.net_days},
    )
...
@router.get(
    "/suppliers/{supplier_id}/tax-identifiers",
    response_model=list[SupplierTaxIdentifierResponse],
    dependencies=[Depends(require_permission("suppliers:read"))],
)
async def list_tax_identifiers(
    supplier_id: int, request: Request, session: SessionDep, current: CurrentUser
) -> list[SupplierTaxIdentifierResponse]:
    company_id = request_company_id(request)
    await require_company_access(session, current, company_id)
    repo = _repo(session)
...
@router.post(
    "/suppliers/{supplier_id}/tax-identifiers",
    response_model=SupplierTaxIdentifierResponse,
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(require_permission("suppliers:tax_identifiers"))],
)
async def create_tax_identifier(
    supplier_id: int,
    ...
    duplicate = await session.scalar(
        select(SupplierTaxIdentifierModel).where(
            SupplierTaxIdentifierModel.supplier_id == supplier_id,
            SupplierTaxIdentifierModel.country_id == payload.country_id,
            SupplierTaxIdentifierModel.identifier_type == payload.identifier_type,
            SupplierTaxIdentifierModel.normalized_value == normalized,
        )
    )
    if duplicate:
        raise HTTPException(status_code=409, detail="El identificador fiscal ya existe")
    item = SupplierTaxIdentifierModel(
        supplier_id=supplier_id, normalized_value=normalized, **payload.model_dump()
    )
    ...
    await audit.record(
        ...
        user_id=current.id,
        company_id=company_id,
        resource_type="supplier_tax_identifiers",
        resource_id=str(item.id),
        after_state={
...
@router.patch(
    "/suppliers/{supplier_id}/tax-identifiers/{identifier_id}",
    response_model=SupplierTaxIdentifierResponse,
    dependencies=[Depends(require_permission("suppliers:tax_identifiers"))],
)
async def update_tax_identifier(
    supplier_id: int,
    identifier_id: uuid.UUID,
    payload: SupplierTaxIdentifierUpdate,
    request: Request,
    session: SessionDep,
    current: CurrentUser,
    audit: AuditService = Depends(get_audit_service),
...
async def delete_tax_identifier(
    supplier_id: int,
    identifier_id: uuid.UUID,
    request: Request,
    session: SessionDep,
    ...
    await audit.record(
        ...
        after_state={"supplier_id": supplier_id},
    )
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(require_permission("suppliers:addresses"))],
)
async def create_address(
    supplier_id: int,
    payload: SupplierAddressCreate,
    request: Request,
    session: SessionDep,
    current: CurrentUser,
    audit: AuditService = Depends(get_audit_service),
    ...
    await audit.record(
        ...
        user_id=current.id,
        company_id=company_id,
        resource_type="supplier_addresses",
        resource_id=str(item.id),
        after_state={
            "supplier_id": supplier_id,
...
@router.get(
    ...
    dependencies=[Depends(require_permission("suppliers:bank_accounts"))],
)
...
async def create_bank_account(
    supplier_id: int,
    payload: SupplierBankAccountCreate,
    request: Request,
    session: SessionDep,
    current: CurrentUser,
    audit: AuditService = Depends(get_audit_service),
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail="El cifrado de datos bancarios no está configurado"
        ) from exc
    item = SupplierBankAccountModel(
        supplier_id=supplier_id,
        **payload.model_dump(exclude={"account_number", "iban"}),
        **bank_fields,
    )
    ...
    await audit.record(
        ...
        user_id=current.id,
        company_id=company_id,
        resource_type="supplier_bank_accounts",
        resource_id=str(item.id),
        after_state={
            "supplier_id": supplier_id,
...
async def update_bank_account(
    ...
    payload: SupplierBankAccountUpdate,
    request: Request,
    session: SessionDep,
    current: CurrentUser,
    audit: AuditService = Depends(get_audit_service),
