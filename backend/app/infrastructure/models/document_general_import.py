...
class DocumentGeneralImportModel(UUIDPKMixin, TimestampMixin, Base):
    ...
    parent_id: Mapped[uuid.UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("document_general_entries.id", ondelete="SET NULL")
    )
    root_entry_id: Mapped[uuid.UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("document_general_entries.id", ondelete="SET NULL")
    )
...
class DocumentGeneralImportItemModel(UUIDPKMixin, TimestampMixin, Base):
    ...
    import_id: Mapped[uuid.UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("document_general_imports.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    document_id: Mapped[uuid.UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("document_records.id", ondelete="SET NULL")
    )
    ...
    __table_args__ = (
        Index("ix_document_general_import_items_import_status", "import_id", "status"),
        Index("ix_document_general_import_items_document", "document_id"),
    )
