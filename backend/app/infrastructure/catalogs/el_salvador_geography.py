"""División político-administrativa vigente de El Salvador.

Fuente: Decreto Legislativo n.º 762, Ley Especial para la Reestructuración
Municipal (2023): 14 departamentos, 44 municipios y 262 distritos.
"""
...
def catalog_counts() -> tuple[int, int, int]:
    """Return department, municipality and district counts."""
    municipalities = sum(len(items) for items in GEOGRAPHY.values())
    districts = sum(len(districts) for items in GEOGRAPHY.values() for districts in items.values())
    return len(GEOGRAPHY), municipalities, districts
