...
def _import_to_domain(model: DocumentGeneralImportModel) -> DocumentGeneralImport:
    return DocumentGeneralImport(
        id=model.id, company_id=model.company_id, actor_id=model.actor_id,
        parent_id=model.parent_id, root_entry_id=model.root_entry_id,
        status=model.status, total_files=model.total_files, total_folders=model.total_folders,
        total_entries=model.total_entries, total_bytes=model.total_bytes,
        metadata=dict(model.metadata_json or {}), completed_files=model.completed_files, skipped_files=model.skipped_files,
        failed_files=model.failed_files, created_at=model.created_at, updated_at=model.updated_at,
        completed_at=model.completed_at, expires_at=model.expires_at,
    )

def _item_to_domain(model: DocumentGeneralImportItemModel) -> DocumentGeneralImportItem:
    return DocumentGeneralImportItem(
        id=model.id, import_id=model.import_id, kind=model.kind,
        source_path=model.source_path, source_name=model.source_name,
        resolved_path=model.resolved_path, resolved_name=model.resolved_name,
        parent_folder_id=model.parent_folder_id, entry_id=model.entry_id,
        document_id=model.document_id, size_bytes=model.size_bytes,
        content_type=model.content_type, extension=model.extension,
        status=model.status, failure_code=model.failure_code,
        failure_message=model.failure_message, attempts=model.attempts,
        created_at=model.created_at, updated_at=model.updated_at,
    )

def _copy_import(source: DocumentGeneralImport, target: DocumentGeneralImportModel) -> None:
    for field in (
        "company_id", "actor_id", "parent_id", "root_entry_id", "status", "metadata",
        "total_files", "total_folders", "total_entries", "total_bytes",
        "completed_files", "skipped_files", "failed_files", "completed_at", "expires_at",
    ):
...
def _copy_item(source: DocumentGeneralImportItem, target: DocumentGeneralImportItemModel) -> None:
    for field in (
        "import_id", "kind", "source_path", "source_name", "resolved_path", "resolved_name",
        "parent_folder_id", "entry_id", "document_id", "size_bytes", "content_type",
        "extension", "status", "failure_code", "failure_message", "attempts",
    ):
...
class SqlAlchemyDocumentGeneralImportRepository:
    ...
    async def get_import(self, company_id: uuid.UUID, import_id: uuid.UUID) -> DocumentGeneralImport | None:
    ...
    async def get_item(self, company_id: uuid.UUID, import_id: uuid.UUID, item_id: uuid.UUID) -> DocumentGeneralImportItem | None:
        model = await self._session.scalar(
            select(DocumentGeneralImportItemModel)
            .join(DocumentGeneralImportModel, DocumentGeneralImportModel.id == DocumentGeneralImportItemModel.import_id)
            .where(
                DocumentGeneralImportModel.company_id == company_id,
    ...
    async def list_items(self, company_id: uuid.UUID, import_id: uuid.UUID, *, page: int, size: int) -> Sequence[DocumentGeneralImportItem]:
        statement = (
            select(DocumentGeneralImportItemModel)
            .join(DocumentGeneralImportModel, DocumentGeneralImportModel.id == DocumentGeneralImportItemModel.import_id)
            .where(
