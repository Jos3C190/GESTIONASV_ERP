...
async def test_check_permission_superuser_always_passes() -> None:
    ...
    result = await uc.execute(admin.id, COMPANY_ID, "anything:whatever")
...
async def test_check_permission_non_superadmin_granted() -> None:
    ...
    role = await roles.add(Role(id=uuid.uuid4(), company_id=COMPANY_ID, name="ADMIN"))
    all_perms = list(await perms.list_all())
    roles.register_perms(all_perms)
    await roles.set_permissions(COMPANY_ID, role.id, {perm_ids["users:read"]})
    user = await users.add(_make_user(superuser=False))
    await roles.assign_role_to_user(user.id, COMPANY_ID, role.id, uuid.uuid4())
    uc = CheckPermissionUseCase(users, roles)
    result = await uc.execute(user.id, COMPANY_ID, "users:read")
    assert result.allowed
...
async def test_check_permission_not_granted() -> None:
    ...
    user = await users.add(_make_user(superuser=False))
    uc = CheckPermissionUseCase(users, roles)
    result = await uc.execute(user.id, COMPANY_ID, "users:delete")
...
async def test_effective_permissions_superuser_returns_star() -> None:
    ...
    result = await uc.execute(admin.id, COMPANY_ID)
    assert result == ("*",)
...
async def test_effective_permissions_union_of_roles() -> None:
    ...
    r1 = await roles.add(Role(id=uuid.uuid4(), company_id=COMPANY_ID, name="R1"))
    r2 = await roles.add(Role(id=uuid.uuid4(), company_id=COMPANY_ID, name="R2"))
    all_perms = list(await perms.list_all())
    roles.register_perms(all_perms)
    await roles.set_permissions(COMPANY_ID, r1.id, {perm_ids["users:read"]})
    await roles.set_permissions(COMPANY_ID, r2.id, {perm_ids["employees:read"]})
    user = await users.add(_make_user(superuser=False))
    await roles.assign_role_to_user(user.id, COMPANY_ID, r1.id, uuid.uuid4())
    await roles.assign_role_to_user(user.id, COMPANY_ID, r2.id, uuid.uuid4())
    uc = GetEffectivePermissionsUseCase(users, roles)
    result = await uc.execute(user.id, COMPANY_ID)
...
async def test_create_role_duplicate_name() -> None:
    roles = InMemoryRoleRepository()
    uc = CreateRoleUseCase(roles)
    await uc.execute(CreateRoleInput(company_id=COMPANY_ID, name="DUP"))
...
async def test_list_roles_page_applies_limit_and_search() -> None:
    ...
    for name in ("ADMIN", "EMPLOYEE", "MANAGER"):
        await CreateRoleUseCase(roles).execute(CreateRoleInput(company_id=COMPANY_ID, name=name))
...
async def test_delete_system_role_forbidden() -> None:
    ...
    with pytest.raises(BusinessRuleError) as exc:
        await DeleteRoleUseCase(roles).execute(COMPANY_ID, role.id)
