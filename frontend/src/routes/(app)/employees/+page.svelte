<script lang="ts">
  ...
  async function loadData() {
    ...
    try {
      const [empResult, deptResult] = await Promise.all([
        api.employees.list({ page: untrack(() => page), size, search: globalSearch.query || undefined, department_id: deptFilter || undefined, status: statusFilter || undefined }),
  ...
  async function openLink(e: EmployeeOut) {
    linkEmp = e; showLinkModal = true; linkError = null; fLinkUserId = '';
    try { const r = await api.users.list({ size: 100 }); users = r.items; } catch { users = []; }
  }
  function closeLinkModal() { showLinkModal = false; linkEmp = null; linkError = null; }

  async function handleLink() {
    if (!linkEmp || !fLinkUserId) return;
    linkLoading = true; linkError = null;
    try {
      await api.employees.linkUser(linkEmp.id, fLinkUserId);
      closeLinkModal(); await Promise.all([loadData(), loadKpis()]);
    } catch (err) { linkError = err instanceof HttpError ? err.message : 'Error.'; }
  ...
  async function deleteEmp(e: EmployeeOut) {
    ...
    actionLoading = e.id;
    try { await api.employees.delete(e.id); await Promise.all([loadData(), loadKpis()]); }
    catch (err) { error = err instanceof HttpError ? err.message : 'Error.'; }
  ...
  async function unlinkEmp(e: EmployeeOut) {
    ...
    actionLoading = e.id;
    try { await api.employees.unlinkUser(e.id); await Promise.all([loadData(), loadKpis()]); }
    catch (err) { error = err instanceof HttpError ? err.message : 'Error.'; }
    finally { actionLoading = null; }
  }
...
<div class="p-6 md:p-8">
  ...
  <div class="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="rounded-xl border border-border bg-surface-elevated p-4 md:p-5 shadow-sm flex flex-col justify-between h-[120px]">
      <div class="flex items-center justify-between">
        <span class="text-[10.5px] font-bold uppercase tracking-wider text-foreground-subtle">Total empleados</span>
    ...
    <div class="rounded-xl border border-border bg-surface-elevated p-4 md:p-5 shadow-sm flex flex-col justify-between h-[120px]">
      <div class="flex items-center justify-between">
        <span class="text-[10.5px] font-bold uppercase tracking-wider text-foreground-subtle">Activos</span>
    ...
    <div class="rounded-xl border border-border bg-surface-elevated p-4 md:p-5 shadow-sm flex flex-col justify-between h-[120px]">
      <div class="flex items-center justify-between">
        <span class="text-[10.5px] font-bold uppercase tracking-wider text-foreground-subtle">En vacaciones</span>
      ...
      <div>
        <div class="font-mono text-2xl font-bold tabular-nums text-foreground">
          {#if loading}<span class="inline-block h-7 w-8 rounded skeleton"></span>{:else}{kpiVacations}{/if}
        </div>
    ...
    <div class="rounded-xl border border-border bg-surface-elevated p-4 md:p-5 shadow-sm flex flex-col justify-between h-[120px]">
      <div class="flex items-center justify-between">
        <span class="text-[10.5px] font-bold uppercase tracking-wider text-foreground-subtle">Usuarios vinculados</span>
        <div class="font-mono text-lg font-bold text-foreground">
          {#if loading}<span class="inline-block h-5 w-12 rounded skeleton"></span>{:else}{kpiLinked} <span class="text-xs font-normal text-foreground-subtle">/ {kpiTotal}</span>{/if}
        </div>
      </div>
      <div class="flex items-center gap-3">
        <!-- Mini Anillo SVG -->
        <svg width="40" height="40" viewBox="0 0 40 40" class="-rotate-90 flex-none" aria-hidden="true">
          <circle cx="20" cy="20" r={ringR} fill="none" stroke="rgb(var(--border))" stroke-width="4.5"/>
          <circle
            cx="20" cy="20" r={ringR} fill="none" stroke="rgb(var(--primary))" stroke-width="4.5"
            stroke-dasharray={ringC.toFixed(1)} stroke-dashoffset={ringOffset.toFixed(1)} stroke-linecap="round"
        ...
        <div class="text-[11px] space-y-0.5 text-foreground-muted">
          ...
          <p><strong class="font-semibold text-foreground-subtle">{kpiTotal - kpiActive - kpiVacations}</strong> inactivos/baja</p>
        </div>
      </div>
    </div>
          ...
          {#each employees as emp (emp.id)}
            <tr class="hover:bg-surface-muted">
              ...
              <td class="px-2 py-3 text-center">
                <KebabMenu
                  items={[
                    { id: 'detail', label: 'Ver detalle', icon: 'detail', onClick: () => goto(`/employees/${emp.id}`) },
                    { id: 'edit', label: 'Editar', icon: 'edit', onClick: () => goto(`/employees/${emp.id}/edit`) },
                    { id: 'link', label: emp.user_id ? 'Desvincular usuario' : 'Vincular usuario', icon: emp.user_id ? 'unlink' : 'link', onClick: () => emp.user_id ? unlinkEmp(emp) : openLink(emp) },
                    { id: 'delete', label: 'Eliminar', icon: 'delete', variant: 'danger', onClick: () => deleteEmp(emp) },
                  ]}
                />
              </td>
            </tr>
...
<Modal open={showLinkModal} title="Vincular usuario" onclose={closeLinkModal}>
  <form onsubmit={(e) => { e.preventDefault(); handleLink(); }} class="space-y-4">
    ...
    <p class="text-sm text-foreground-muted">Selecciona el usuario a vincular con <strong>{linkEmp?.first_name} {linkEmp?.last_name}</strong></p>
    ...
    <div class="flex justify-end gap-2 pt-2"><Button variant="secondary" onclick={closeLinkModal}>Cancelar</Button><Button type="submit" disabled={linkLoading || !fLinkUserId}>{linkLoading ? 'Vinculando...' : 'Vincular'}</Button></div>
  </form>
</Modal>