<script lang="ts">
  ...
  $effect(() => {
    if (!warehouseId || !canImport || loadedWarehouseId === warehouseId) return;
...
<svelte:head><title>Importar ubicaciones — GestionaSV</title></svelte:head>
...
<div class="space-y-5 p-6 md:p-8">
  <header class="flex items-start gap-3">
    ...
    <div>
      <h1 class="text-xl font-bold text-foreground">Importar ubicaciones</h1>
      <p class="mt-1 text-sm text-foreground-muted">
        Cargue un archivo, valide todas las filas y publique los cambios de forma segura.
      </p>
    </div>
  </header>
    ...
    <div role="alert">
      <Card class="p-6">
        <h2 class="text-sm font-semibold text-danger">
          No tiene permiso para importar ubicaciones
        </h2>
        <p class="mt-1 text-xs leading-5 text-foreground-muted">
          Solicite el permiso <span class="font-mono">locations.import</span> para continuar.
        </p>
        <div class="mt-4">
  ...
  {:else if loading}
    <div class="h-96 rounded-2xl skeleton" aria-label="Cargando importador"></div>
  {:else if error}
    <div role="alert">
      <Card class="p-6">
        <h2 class="text-sm font-semibold text-danger">No se pudo preparar el importador</h2>
