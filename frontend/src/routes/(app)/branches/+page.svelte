<script lang="ts">
  ...
  let ringOffset = $derived(totalBranches > 0 ? ringC - (activeCount / totalBranches) * ringC : ringC);
</script>
  ...
  <div class="grid flex-none grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="rounded-xl border border-border bg-surface-elevated p-4 md:p-5 shadow-sm flex flex-col justify-between h-[120px]">
      <div class="flex items-center justify-between">
        <span class="text-[10.5px] font-bold uppercase tracking-wider text-foreground-subtle">Empleados</span>
      ...
      <div>
        <div class="font-mono text-2xl font-bold tabular-nums text-foreground">{totalEmployees}</div>
        <p class="text-[11px] text-foreground-subtle mt-1 truncate">En {totalBranches} sucursales</p>
      </div>
    </div>
    ...
    <div class="rounded-xl border border-border bg-surface-elevated p-4 md:p-5 shadow-sm flex flex-col justify-between h-[120px]">
      <div class="flex items-center justify-between">
        <span class="text-[10.5px] font-bold uppercase tracking-wider text-foreground-subtle">Almacenes</span>
      ...
      <div>
        <div class="font-mono text-2xl font-bold tabular-nums text-foreground">{totalWarehouses}</div>
    ...
    <div class="rounded-xl border border-border bg-surface-elevated p-4 md:p-5 shadow-sm flex flex-col justify-between h-[120px]">
      <div class="flex items-center justify-between">
        <span class="text-[10.5px] font-bold uppercase tracking-wider text-foreground-subtle">Ventas del mes</span>
      ...
      <div>
        <div class="font-mono text-2xl font-bold tabular-nums text-foreground">${totalSales.toLocaleString()}</div>
        <p class="text-[11px] text-foreground-subtle mt-1 truncate">Consolidado general</p>
      </div>
    </div>
    ...
    <div class="rounded-xl border border-border bg-surface-elevated p-4 md:p-5 shadow-sm flex flex-col justify-between h-[120px]">
      <div class="flex items-center justify-between">
        <span class="text-[10.5px] font-bold uppercase tracking-wider text-foreground-subtle">Sucursales activas</span>
        <div class="font-mono text-lg font-bold text-foreground">{activeCount} <span class="text-xs font-normal text-foreground-subtle">/ {totalBranches}</span></div>
      </div>
      <div class="flex items-center gap-3">
        ...
        <svg width="40" height="40" viewBox="0 0 40 40" class="-rotate-90 flex-none" aria-hidden="true">
          ...
          <circle
            cx="20" cy="20" r={ringR} fill="none" stroke="rgb(var(--success))" stroke-width="4.5"
        ...
        <div class="text-[11px] space-y-0.5 text-foreground-muted">
          <p><strong class="font-semibold text-warning">{maintenanceCount}</strong> en mantenimiento</p>
          <p><strong class="font-semibold text-foreground-subtle">{inactiveCount}</strong> inactiva</p>
        </div>
      </div>
    </div>
