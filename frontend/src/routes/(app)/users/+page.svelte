<script lang="ts">
  ...
  async function loadUsers() {
    ...
    try {
      const [result, roleCatalogue, employeePage] = await Promise.all([
        api.users.list({ page, size, search: globalSearch.query || undefined }),
        permissions.hasPermission('roles:read') ? api.roles.list() : Promise.resolve([]),
      ...
      rolesByUser = Object.fromEntries(
        permissions.hasPermission('roles:read')
          ? await Promise.all(
              items.map(async (user) => [user.id, await api.roles.userRoles(user.id)])
            )
          : []
      );
      meta = result.meta;
  ...
  function openEdit(user: UserOut) {
    ...
    fEmployeeId = employeeByUser[user.id]?.id ?? '';
    originalEmployeeId = fEmployeeId;
  }
  ...
  async function handleSubmit() {
    ...
    try {
      if (modalMode === 'create') {
        ...
        if (permissions.hasPermission('employees:update') && fEmployeeId !== originalEmployeeId) {
          if (originalEmployeeId) await api.employees.unlinkUser(originalEmployeeId);
          if (fEmployeeId) await api.employees.linkUser(fEmployeeId, modalUser.id);
        }
  ...
  let availableEmployees = $derived(
    employees.filter((employee) => !employee.user_id || employee.id === originalEmployeeId)
  );
