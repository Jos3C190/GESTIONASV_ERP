<script lang="ts">
  ...
  import type { BranchImage } from '../types';
  ...
  function requestRemove(index: number) {
    ...
    confirmation.request({
      kind: 'delete',
      title: 'Quitar imagen de la sucursal',
      description: 'La imagen se marcará para eliminarse cuando guarde los cambios de la sucursal.',
...
<section class="space-y-3 rounded-xl border border-border bg-surface-muted/30 p-4">
  <div class="flex items-start justify-between gap-3">
    <div>
      <h3 class="text-sm font-semibold text-foreground">Galería de la sucursal</h3>
      <p class="mt-1 text-xs text-foreground-muted">
        La primera imagen será la portada usada en el mapa.
      </p>
    </div>
  ...
  {#if images.length === 0}<div
    ...
    >
      No hay imágenes. Agregue una portada o fotografías del local.
    </div>{/if}
  {#each images as image, index (index)}
    <div class="rounded-xl border border-border bg-surface-elevated p-3">
      <ImageUpload
        id={`branch-image-${index}`}
        label={index === 0 ? 'Portada' : `Imagen ${index + 1}`}
        purpose="branch_image"
        {companyId}
        bind:value={image.url}
        bind:publicId={image.public_id}
        alt={image.caption || 'Sucursal'}
        shape="wide"
      />
      <label for={`branch-caption-${index}`} class="mt-3 block text-xs font-medium text-foreground"
        >Descripción accesible</label
      >
      <input
        id={`branch-caption-${index}`}
