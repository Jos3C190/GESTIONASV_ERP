<script lang="ts">
  ...
  let { title, description, data, onselect }: Props = $props();
  const palette = ['0 112 243', '59 130 246', '14 165 233', '100 116 139', '148 163 184', '34 197 94', '245 158 11'];
  const size = 124;
  const stroke = 14;
...
<div class="rounded-2xl border border-border bg-surface-elevated p-4 shadow-soft transition-all duration-200 hover-lift">
  <div class="mb-3">
    <h2 class="text-sm font-semibold text-foreground">{title}</h2>
    <p class="mt-0.5 text-xs text-foreground-muted">{description}</p>
  </div>
  {#if total === 0}
    <div class="flex min-h-[132px] items-center justify-center rounded-lg border border-dashed border-border text-xs text-foreground-muted">
      Sin datos para este filtro
    </div>
  {:else}
    <div class="flex min-h-[132px] items-center gap-3">
      <svg
        width={size}
        height={size}
      ...
      >
        ...
        <text
          ...
          style="font-size: 8px; text-transform: uppercase; letter-spacing: 0.06em;"
        >productos</text>
      </svg>
          ...
          {#if segment.filterable}<button
            ...
            ><span
              ...
              ></span><span class="min-w-0 flex-1 truncate text-xs text-foreground"
                >{segment.label}</span
              ><span class="font-mono text-xs tabular-nums text-foreground-muted">{segment.value}</span
              ></button
            >{:else}<div class="flex items-center gap-2 px-1 py-1">
              <span class="h-2.5 w-2.5 flex-none rounded-sm" style={`background: rgb(${segment.color});`}></span>
              <span class="min-w-0 flex-1 truncate text-xs text-foreground-muted">{segment.label}</span>
              <span class="font-mono text-xs tabular-nums text-foreground-muted">{segment.value}</span>
            </div>{/if}
        {/each}
      </div>
