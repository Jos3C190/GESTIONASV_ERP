<script lang="ts">
  ...
  import type { PurchaseRequest, PurchaseRequestDetailInput } from '$lib/types/purchase-request';
  ...
  function fromRequest(item: PurchaseRequest) {
    branchId = item.branch_id;
    warehouseId = item.warehouse_id;
    requiredDate = item.required_date ? item.required_date.slice(0, 10) : '';
  ...
  async function loadWarehouses(selectedBranchId: string) {
    ...
    warehouses = response.items.filter(
  ...
  async function loadData(id: string) {
    ...
    try {
      const [item, productPage] = await Promise.all([
      ...
      if (item.status !== 'draft') {
        editable = false;
        error = 'Solo las solicitudes en borrador pueden editarse.';
        return;
      }

      await loadWarehouses(item.branch_id);
    } catch (err: unknown) {
      if (currentGeneration !== generation) return;
  ...
  async function submit() {
    if (!canManage || !editable || saving || !validate()) return;

    saving = true;
    error = null;

    try {
      const updated = await purchaseRequestsApi.update(requestId, {
        branch_id: branchId,
      ...
      await goto(`/purchase-requests/${updated.id}`);
    } catch (err: unknown) {
      error =
        err instanceof HttpError
          ? err.message
...
<div class="min-h-full bg-background px-4 pb-8 pt-4 sm:px-6 sm:pt-6 md:px-8 md:pt-8">
  <header class="mb-6 flex items-center gap-3 border-b border-border pb-4">
    <a
      ...
      aria-label="Volver al detalle de solicitud"
    >
      <svg
        width="18"
        height="18"
    ...
    <div>
      <h1 class="text-xl font-semibold text-foreground">Editar solicitud de compra</h1>
      <p class="mt-1 text-sm text-foreground-muted">Actualiza la información del borrador.</p>
    </div>
  </header>
  ...
  {:else if !editable}
    <div
      ...
      role="alert"
    >
      {error ?? 'La solicitud no puede editarse.'}
    </div>
  {:else}
    {#if error}
      <div
    ...
    >
      <Card class="p-5">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <div>
            <label
              for="purchase-request-edit-branch"
              class="mb-1 block text-sm font-medium text-foreground"
            >
              Sucursal <span class="text-danger">*</span>
            </label>
            <select
              id="purchase-request-edit-branch"
              aria-label="Sucursal"
              value={branchId}
          ...
          <div>
            <label
              for="purchase-request-edit-warehouse"
              class="mb-1 block text-sm font-medium text-foreground"
            >
              Almacén <span class="text-danger">*</span>
            </label>
            <select
              id="purchase-request-edit-warehouse"
              aria-label="Almacén"
          ...
          <div>
            <label
              for="purchase-request-edit-required-date"
              class="mb-1 block text-sm font-medium text-foreground"
            >
              Fecha requerida
            </label>
            <input
              id="purchase-request-edit-required-date"
          ...
          <div>
            <label
              for="purchase-request-edit-justification"
              class="mb-1 block text-sm font-medium text-foreground"
            >
              Justificación <span class="text-danger">*</span>
            </label>
            <input
              id="purchase-request-edit-justification"
        ...
        <div class="mt-4">
          <label
            for="purchase-request-edit-notes"
            class="mb-1 block text-sm font-medium text-foreground"
          >
            Notas
          </label>
          <textarea
            id="purchase-request-edit-notes"
            aria-label="Notas"
            bind:value={notes}
            maxlength="4000"
            rows="3"
        ...
        <div class="divide-y divide-border">
          {#each lines as line, index (index)}
            <div class="grid gap-3 p-4 lg:grid-cols-[2fr_1fr_2fr_2fr_auto] lg:items-start">
              <div>
                <label
                  for={`purchase-request-edit-product-${index}`}
                  class="mb-1 block text-sm font-medium text-foreground"
                >
                  Producto {index + 1} <span class="text-danger">*</span>
                </label>
                <select
                  id={`purchase-request-edit-product-${index}`}
                ...
                >
                  ...
                  {#each products as product}
                    <option value={String(product.id_product)}
              ...
              <div>
                <label
                  for={`purchase-request-edit-quantity-${index}`}
                  class="mb-1 block text-sm font-medium text-foreground"
                >
                  Cantidad <span class="text-danger">*</span>
                </label>
                <input
                  id={`purchase-request-edit-quantity-${index}`}
              ...
              <div>
                <label
                  for={`purchase-request-edit-description-${index}`}
                  class="mb-1 block text-sm font-medium text-foreground"
                >
                  Descripción
                </label>
                <input
                  id={`purchase-request-edit-description-${index}`}
              ...
              <div>
                <label
                  for={`purchase-request-edit-line-notes-${index}`}
                  class="mb-1 block text-sm font-medium text-foreground"
                >
                  Notas
                </label>
                <input
                  id={`purchase-request-edit-line-notes-${index}`}
      ...
      <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        <Button
          type="button"
          variant="secondary"
          onclick={() => goto(`/purchase-requests/${requestId}`)}
        >
          Cancelar
        </Button>
